...
const Icon = {
  ...
  spinner: (p: React.SVGProps<SVGSVGElement>) => (
    <svg fill="none" className="animate-spin" viewBox="0 0 24 24" {...p}>
    ...
    <div className="min-h-screen bg-[#F4F1EA] font-body text-[15px] text-[--foreground]">
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-between gap-4 border-t border-brand-light bg-white/95 px-5 py-3 backdrop-blur-md lg:hidden">
        ...
        <button
          ...
          className="rounded-full bg-brand-dark px-6 py-3 text-sm font-semibold text-white shadow-sm transition-all hover:bg-brand-darkest hover:shadow-md active:scale-[0.98] cursor-pointer inline-flex items-center gap-2"
        >
          Reserve Your Spot
        </button>
      </div>
...
function BookingCard({ startingPrice, guests, setGuests, total, onBookNow, title, slug }: { startingPrice: number | null; guests: number; setGuests: (n: number) => void; total: number; onBookNow: () => void; title: string; slug: string }) {
  return (
    <div className="ticket rounded-2xl border border-brand-light bg-white p-6 shadow-[0_20px_50px_-20px_rgba(27,77,27,0.3)]">
      ...
      <button
        onClick={onBookNow}
        className="mt-5 w-full rounded-full bg-brand-dark py-3.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-brand-darkest hover:shadow-md active:scale-[0.99] cursor-pointer flex items-center justify-center gap-2"
      >
        Reserve Your Spot
      </button>
